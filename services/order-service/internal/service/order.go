...
//
// مراحل:
//  1. دریافت همزمان اطلاعات محصولات از سرویس محصولات از طریق errgroup.
//  2. اگر رزرو یک آیتم شکست بخورد، تمام آیتم‌های قبلی که تا این
//     لحظه با موفقیت رزرو شده‌اند آزاد میشود
//     جهت مدیریت دقیق Compensation در صورت بروز خطا).
//  3. ثبت سفارش و آیتم‌ها در یک تراکنش واحد دیتابیس.
//  4. در صورت شکست در دیتابیس، آزاد کردن رزروهای انجام‌شده.
//  5. انتشار رویداد order.created جهت شروع فرایند پرداخت.
func (
	s *OrderService,
) CreateOrder(
	ctx context.Context,
